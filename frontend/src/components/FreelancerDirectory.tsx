...
export const FreelancerDirectory: React.FC<FreelancerDirectoryProps> = ({
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in text-white">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800/90 pb-4">
        <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredProfiles.map((p) => {
            let tierBadge = (
              <span className="bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider">
                🚀 Rising Talent
              </span>
            );
            if (p.tier === "Apex Pioneer") {
              tierBadge = (
                <span className="bg-violet-500/20 text-violet-300 border border-violet-500/40 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider shadow-[0_0_15px_rgba(168,85,247,0.25)]">
                  💎 Apex Pioneer
                </span>
              );
            } else if (p.tier === "Gold Pro") {
              tierBadge = (
                <span className="bg-amber-500/20 text-amber-300 border border-amber-500/40 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider shadow-[0_0_15px_rgba(251,191,36,0.25)]">
                  🥇 Gold Pro
                </span>
              );
            } else if (p.tier === "Silver Contributor") {
              tierBadge = (
                <span className="bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider">
                  🥈 Silver Talent
                </span>
              );
            }
            ...
            return (
              <div key={p.address} className="glass-panel-interactive p-6 rounded-2xl flex flex-col justify-between gap-5 border border-slate-800">
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-start gap-2">
                    {tierBadge}
                    <div className="flex items-center gap-1.5 bg-slate-950 px-3 py-1 rounded-full border border-slate-800 text-xs shadow-inner">
                      <span className="text-amber-400 font-bold">★</span>
                      <span className="font-black text-white">
                        {p.averageRating ? p.averageRating.toFixed(1) : "N/A"}
                      </span>
                      <span className="text-slate-400 text-[10px] font-mono">({p.ratingCount})</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 bg-slate-950/80 p-3.5 rounded-xl border border-slate-800 text-xs">
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block font-mono">Total Gigs</span>
                    <span className="font-mono font-black text-white">{p.totalGigs}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold uppercase block font-mono">Completed</span>
                    <span className="font-mono font-black text-emerald-400">{p.completedGigs}</span>
                  </div>
                </div>

                <button
                  onClick={() => onSelectFreelancer(p.address)}
                  className="w-full bg-gradient-to-r from-cyan-400 via-sky-400 to-violet-500 hover:from-cyan-300 hover:to-violet-400 text-slate-950 font-black py-3 rounded-xl transition text-xs shadow-[0_0_20px_rgba(0,242,254,0.3)] transform hover:-translate-y-0.5"
                >
                  ⚡ Hire Talent for Gig
                </button>
              </div>
